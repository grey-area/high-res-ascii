...
lower_res_image = ndimage.interpolation.zoom(gray_image, float(width) / image.shape[0])

# Normalize so that the whole range of characters is used
upper_limit = lower_res_image.max()*((len(characters)+1)/float(len(characters)))
lower_limit = lower_res_image.min()
brightnesses = ((brightnesses - brightnesses.min()) / (brightnesses.max() - brightnesses.min() )) * (upper_limit - lower_limit) + lower_limit

# Construct the image pixel-by-pixel, using the character whose brightness is closest to the pixel value
with open('image.txt','w') as f:
    for row in lower_res_image:
        for c in row:
            character_index = bisect.bisect(brightnesses, c)
            f.write('%s' % characters[character_index])
        f.write('\n')
